...
public class UrgenciaDAO {
    ...
    public boolean crear(Urgencia urgencia) {
        String sql = "INSERT INTO urgencias (id, paciente_id, empleado_id, nivel_triage, sintomas, ubicacion, signos_vitales, estado, fecha_registro, ambulancia_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = dbService.getConnection();
            ...
            stmt.setString(6, urgencia.getUbicacion());
            stmt.setString(7, urgencia.getTriage() != null ? urgencia.getTriage().getSignosVitales() : null);
            stmt.setString(8, urgencia.getEstado());
            stmt.setTimestamp(9, urgencia.getHoraReporte() != null ? Timestamp.valueOf(urgencia.getHoraReporte()) : Timestamp.valueOf(LocalDateTime.now()));
            stmt.setString(10, urgencia.getAmbulanciaAsignada() != null ? urgencia.getAmbulanciaAsignada().getId() : null);
            stmt.executeUpdate();
            return true;
    ...
    public Urgencia buscarPorId(String id) {
        String sql = "SELECT u.*, p.nombre_completo as paciente_nombre, e.nombre_completo as empleado_nombre, a.id as ambulancia_id, a.placa as ambulancia_placa, a.estado as ambulancia_estado FROM urgencias u LEFT JOIN pacientes p ON u.paciente_id = p.id LEFT JOIN empleados e ON u.empleado_id = e.id LEFT JOIN ambulancias a ON u.ambulancia_id = a.id WHERE u.id = ?";
        try (Connection conn = dbService.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, id);
    ...
    public List<Urgencia> obtenerTodas() {
        ...
        String sql = "SELECT u.*, p.nombre_completo as paciente_nombre, e.nombre_completo as empleado_nombre, a.id as ambulancia_id, a.placa as ambulancia_placa, a.estado as ambulancia_estado FROM urgencias u LEFT JOIN pacientes p ON u.paciente_id = p.id LEFT JOIN empleados e ON u.empleado_id = e.id LEFT JOIN ambulancias a ON u.ambulancia_id = a.id ORDER BY u.fecha_registro DESC";
        try (Connection conn = dbService.getConnection();
    ...
    public List<Urgencia> obtenerPorEstado(String estado) {
        List<Urgencia> urgencias = new ArrayList<>();
        String sql = "SELECT u.*, p.nombre_completo as paciente_nombre, e.nombre_completo as empleado_nombre, a.id as ambulancia_id, a.placa as ambulancia_placa, a.estado as ambulancia_estado FROM urgencias u LEFT JOIN pacientes p ON u.paciente_id = p.id LEFT JOIN empleados e ON u.empleado_id = e.id LEFT JOIN ambulancias a ON u.ambulancia_id = a.id WHERE u.estado = ? ORDER BY u.nivel_triage DESC, u.fecha_registro ASC";
        try (Connection conn = dbService.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public boolean actualizarEstadoConAmbulancia(String id, String estado, String empleadoId, String ambulanciaId) {
        String sql = "UPDATE urgencias SET estado = ?, empleado_id = ?, ambulancia_id = ? WHERE id = ?";
        try (Connection conn = dbService.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, estado);
            stmt.setString(2, empleadoId);
            stmt.setString(3, ambulanciaId);
            stmt.setString(4, id);
            return stmt.executeUpdate() > 0;
        } catch (SQLException e) {
            System.err.println("Error actualizando estado de urgencia con ambulancia: " + e.getMessage());
            return false;
        }
    }

    public boolean actualizarAmbulancia(String id, String ambulanciaId) {
        String sql = "UPDATE urgencias SET ambulancia_id = ? WHERE id = ?";
        try (Connection conn = dbService.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, ambulanciaId);
            stmt.setString(2, id);
            return stmt.executeUpdate() > 0;
        } catch (SQLException e) {
            System.err.println("Error actualizando ambulancia de urgencia: " + e.getMessage());
            return false;
        }
    }

    public boolean actualizar(String id, Urgencia urgencia) {
        String sql = "UPDATE urgencias SET paciente_id = ?, empleado_id = ?, nivel_triage = ?, sintomas = ?, ubicacion = ?, signos_vitales = ?, estado = ? WHERE id = ?";
        try (Connection conn = dbService.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            stmt.setString(5, urgencia.getUbicacion());
    ...
    private Urgencia mapResultSetToUrgencia(ResultSet rs) throws SQLException {
        ...
        urgencia.setUbicacion(rs.getString("ubicacion"));
        ...
        // Cargar ambulancia asignada
        String ambulanciaId = rs.getString("ambulancia_id");
        if (ambulanciaId != null) {
            com.ingenieria.software1.model.Ambulancia ambulancia = new com.ingenieria.software1.model.Ambulancia();
            ambulancia.setId(ambulanciaId);
            ambulancia.setPlaca(rs.getString("ambulancia_placa"));
            ambulancia.setEstado(rs.getString("ambulancia_estado"));
            urgencia.setAmbulanciaAsignada(ambulancia);
        }
